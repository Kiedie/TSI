...
(:requirements :strips  :typing :adl)
    ...
    (:types 
        ...
        Localizacion Recurso Investigacion  tipoInvestigacion  - NoMovible
    )

    (:constants 
        VCE  Marine  Segador                                    - tipoUnidad
        CentroDeMando Barracones Extractor BahiaInvestigacion   - tipoEdificio
        Minerales Gas                                           - tipoRecurso
        ImpulsaSegador                                          - tipoInvestigacion
    ...
    (:predicates 
        ...
        ;PREDICADOS PARA EL EJERCICIO 5

        ;Recursos que necsita una investigación
        (necesitaInvestRec    ?i - tipoInvestigacion ?tipo - tipoRecurso)
        ;La investigación ya se ha realizado
        (investigado          ?i - tipoInvestigacion)
        ;La unidad que necesita de una investigación para ser reclutada
        (necesitaInvestUni    ?u - tipoUnidad    ?i - tipoInvestigacion)
        ;Para hacer distincion sobre el tipo de investigación
        (esTipoInvest         ?i - Investigacion ?tipo - tipoInvestigacion)
    )
    ...
    (:action reclutar
        ...
        :precondition (and
            ...
            (not (exists ( ?loc - Localizacion)(and (en ?u ?loc )) ))
            ...
            (forall (?inv - tipoInvestigacion)                                      ;Para cada tipo de investigacion
                (exists (?tipounidad - tipoUnidad)                                  ;Veo que existe un tipo de unidad tal que
                    (and        
                        (esTipoUnidad         ?u       ?tipounidad )                ;Mi unidad es de ese tipo y 
                        (or
                            (not (necesitaInvestUni    ?tipounidad    ?inv))        ;O no requiere de investigacion
                            (and
                                (necesitaInvestUni    ?tipounidad  ?inv)            ;O si requiere de investigacion
                                (investigado          ?inv)                         ;Ya he investigado
                            )
                        )
                    )
                )
   ...
   (:action investigar
       :parameters (?e - Edificio ?i - tipoInvestigacion)
       :precondition 
       (and 
        (not (investigado ?i))                      ;No hayamos investigado sobre el asunto
        (esTipoEdificio    ?e   BahiaInvestigacion) ;El edificio debe de ser una bahía
        (edifConstruido    ?e)                      ;El edificio debe de estar construido
       
        (forall (?recurso - tipoRecurso)                    ;Para cada tipo de recurso
            (or
                (not (necesitaInvestRec    ?i ?recurso) )   ;O no lo necesitamos
                (and
                    (necesitaInvestRec    ?i ?recurso)      ;O si lo necesitamos
                    (tengoRecurso         ?recurso)         ;Ya lo tenemos
                )
            )
        )
